/*
 * Vendored from foliate-js (https://github.com/johnfactotum/foliate-js/blob/main/mobi.js, MIT).
 * Patched for spec compliance (ADR-0006): the upstream PDB record list is read
 * with an 8-byte stride, but PalmDB record entries are 5 bytes (4-byte offset
 * + 1-byte attribute). With upstream's stride every record after the first
 * misreads, so later chapters render as binary. Fixed to 5 below; report
 * upstream and drop this vendored copy when it is fixed there.
 */
...
class PDB {
    ...
    async open(file) {
        ...
        this.pdb = pdb
        const buffer = await file.slice(78, 78 + pdb.numRecords * 5).arrayBuffer()
        // get start and end offsets for each record (5 bytes: 4 offset + 1 attr)
        this.#offsets = Array.from({ length: pdb.numRecords },
            (_, i) => getUint(buffer.slice(i * 5, i * 5 + 4)))
            .map((x, i, a) => [x, a[i + 1]])
    }
    loadRecord(index) {
